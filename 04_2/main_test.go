...
// doesOneOverlapOther
func testIsThereOverlapHelper(t *testing.T, oneLower, oneUpper, otherLower, otherUpper int, expected bool) {
	one := assignment{LowerBound: oneLower, UpperBound: oneUpper}
	other := assignment{LowerBound: otherLower, UpperBound: otherUpper}
	if isThereOverlap(one, other) != expected {
		t.Fatalf("Expected %t, but received %t for doesOneOverlapOther(%v, %v).", expected, !expected, one, other)
	}
}

func TestIsThereOverlap(t *testing.T) {
	// From exercise example.
	testIsThereOverlapHelper(t, 2, 4, 6, 8, false)
	testIsThereOverlapHelper(t, 6, 8, 2, 4, false)
	testIsThereOverlapHelper(t, 2, 3, 4, 5, false)
	testIsThereOverlapHelper(t, 4, 5, 2, 3, false)
	testIsThereOverlapHelper(t, 5, 7, 7, 9, true)
	testIsThereOverlapHelper(t, 7, 9, 5, 7, true)
	testIsThereOverlapHelper(t, 2, 8, 3, 7, true)
	testIsThereOverlapHelper(t, 3, 7, 2, 8, true)
	testIsThereOverlapHelper(t, 6, 6, 4, 6, true)
	testIsThereOverlapHelper(t, 4, 6, 6, 6, true)
	testIsThereOverlapHelper(t, 2, 6, 4, 8, true)
	testIsThereOverlapHelper(t, 4, 8, 2, 6, true)
}
...
func TestDoesGroupContainRedundantElves(t *testing.T) {
	// true (from example.)
	testAssignments := []assignment{{5, 7}, {7, 9}}
	...
	testAssignments = []assignment{{2, 8}, {3, 7}}
	containsRedundantElves = doesGroupContainRedundantElves(&testAssignments)
	if !containsRedundantElves {
		t.Fatalf("Expected to find redundant elves in %v", testAssignments)
	}
	testAssignments = []assignment{{6, 6}, {4, 6}}
	containsRedundantElves = doesGroupContainRedundantElves(&testAssignments)
	if !containsRedundantElves {
		t.Fatalf("Expected to find redundant elves in %v", testAssignments)
	}
	testAssignments = []assignment{{2, 6}, {4, 8}}
	containsRedundantElves = doesGroupContainRedundantElves(&testAssignments)
	if !containsRedundantElves {
		t.Fatalf("Expected to find redundant elves in %v", testAssignments)
	}
	...
	// false (from example.)
	testAssignments = []assignment{{2, 4}, {6, 8}}
	containsRedundantElves = doesGroupContainRedundantElves(&testAssignments)
	if containsRedundantElves {
		t.Fatalf("Expected to find no redundant elves in %v", testAssignments)
	}
	testAssignments = []assignment{{2, 3}, {4, 5}}
	containsRedundantElves = doesGroupContainRedundantElves(&testAssignments)
	if containsRedundantElves {
		t.Fatalf("Expected to find no redundant elves in %v", testAssignments)
	}
