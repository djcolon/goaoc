...
func (stacks stacks) moveCrates(instruction move) (err error) {
	...
	// Calculate these to make it a bit more readable.
	fromAddress := instruction.from - 1
	toAddress := instruction.to - 1
	fromLength := len(stacks.stacks[fromAddress])
	// Get a stack of crates.
	crates := stacks.stacks[fromAddress][fromLength-instruction.count : fromLength]
	// Shorten the from array.
	stacks.stacks[fromAddress] = stacks.stacks[fromAddress][:fromLength-instruction.count]
	// And append to the to.
	stacks.stacks[toAddress] = append(stacks.stacks[toAddress], crates...)
