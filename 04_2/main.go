...
// Tests whether there is any overlap between assignments.
// Assuming the ranges are continuous, there will always be overlap if the end
// of the range of the one are within the ends of the other.
// See README.md
func doesOneOverlapOther(one, other assignment) (oneContainsOther bool) {
	return (one.LowerBound >= other.LowerBound && one.LowerBound <= other.UpperBound) || (one.UpperBound >= other.LowerBound && one.UpperBound <= other.UpperBound)
}

// Does the same as doesOneOverlapOther, but also tests the reverse.
func isThereOverlap(one, other assignment) (isVOerlap bool) {
	return doesOneOverlapOther(one, other) || doesOneOverlapOther(other, one)
}
...
func doesGroupContainRedundantElves(groupAssignments *[]assignment) (containsRedundantElves bool) {
	...
	for _, combination := range exclusiveCombinations {
		if isThereOverlap((*groupAssignments)[combination[0]], (*groupAssignments)[combination[1]]) {
			return true
		}
	}
	return false
